...
from app.capabilities.software.models import SoftwareSyncCheckpoint
from app.capabilities.software.service import SoftwareSyncService
...
class SoftwareSyncHandler:
    ...
    async def handle(self,envelope,*,queue,worker_id,lease_seconds):
        ...
        cp=self.session.execute(select(SoftwareSyncCheckpoint).where(SoftwareSyncCheckpoint.job_id==job_id)).scalar_one_or_none()
        if cp and cp.status=="completed": return
        if cp is None: cp=SoftwareSyncCheckpoint(job_id=job_id,environment_id=env_id,managed_tenant_id=tenant_id,next_resource="/deviceManagement/detectedApps"); self.session.add(cp); self.session.flush()
        client,service=self.graph_client_factory(env_id,tenant_id),SoftwareSyncService(self.session,environment_id=env_id,managed_tenant_id=tenant_id,sync_id=job_id)
        while cp.next_resource:
            self._validate(cp.next_resource)
            try: response=await client.get(cp.next_resource,params={"$select":"id,displayName,version,sizeInByte,deviceCount,publisher,platform"} if cp.next_resource=="/deviceManagement/detectedApps" else None)
            ...
            try: cp.processed_count+=service.upsert(values)
            except ValueError as exc: raise TerminalJobError("graph_software_invalid") from exc
